      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          if (searchResultIsDm(item)) {
            return (
              <ConversationSearchResultsListItemDm conversationTopic={item.conversationTopic} />
            )
          }
          if (searchResultIsGroup(item)) {
            return (
              <ConversationSearchResultsListItemGroup conversationTopic={item.conversationTopic} />
            )
          }
          if (searchResultIsProfile(item)) {
            return <ConversationSearchResultsListItemUser inboxId={item.inboxId} />
          }
          if (searchResultIsSocialProfile(item)) {
            return <ConversationSearchResultsListItemNoConvosUser socialProfile={item.profile} />
          }
          if (searchResultIsEthAddress(item)) {
            return <ConversationSearchResultsListItemEthAddress ethAddress={item.address} />
          }
