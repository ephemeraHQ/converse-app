...
import {
  keyboardStateListeners,
  useKeyboardStore,
} from "../store/useKeyboardStore";
import type { RecentPicksPersistenceConfig } from "../types";

export const useRecentPicksPersistence = (
  config: RecentPicksPersistenceConfig
) => {
  useEffect(() => {
    ...
    const initialize = async () => {
      try {
        ...
        console.error(
          "there was a problem with initialization of rn-emoji-keyboard recent picks"
        );
        console.error(e);
      }
    };
