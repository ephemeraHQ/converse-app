...
function getGroupPermissionPolicySet(args: { account: string; topic: ConversationTopic }) {
  const { account, topic } = args
  const group = getGroupQueryData({ account, topic })
  return group?.permissionPolicySet()
}

export const useGroupPermissionsQuery = (account: string, topic: ConversationTopic) => {
  return useQuery(getGroupPermissionsQueryOptions({ account, topic }))
}

function getGroupPermissionsQueryOptions(args: { account: string; topic: ConversationTopic }) {
  const { account, topic } = args
  return queryOptions({
    queryFn: () =>
