...
export const useGroupMembers = (topic: ConversationTopic) => {
  ...
  const { mutateAsync: promoteToAdmin } = usePromoteToAdminMutation(account, topic)
  const { mutateAsync: promoteToSuperAdmin } = usePromoteToSuperAdminMutation(account, topic)
  const { mutateAsync: revokeSuperAdmin } = useRevokeSuperAdminMutation(account, topic)

  const { mutateAsync: revokeAdmin } = useRevokeAdminMutation(account, topic)

  const { mutateAsync: removeMember } = useRemoveFromGroupMutation(account, topic)
