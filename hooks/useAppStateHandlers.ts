...
export const useAppStateHandlers = (settings?: AppStateHookSettings) => {
  ...
  const listenerRef = useRef<ReturnType<typeof AppState.addEventListener> | null>(null)

  useEffect(() => {
    const handleAppStateChange: Handler = (nextAppState) => {
      if (nextAppState === "active" && previousAppState.current !== "active") {
        onForeground && onForeground()
      } else if (previousAppState.current === "active" && nextAppState === "background") {
        onBackground && onBackground()
      } else if (previousAppState.current === "active" && nextAppState === "inactive") {
    ...
    listenerRef.current = AppState.addEventListener("change", handleAppStateChange)
