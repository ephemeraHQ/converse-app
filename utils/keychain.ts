...
export const getAccountEncryptionKey = async (account: string): Promise<Buffer> => {
  const existingKey = await getSecureItemAsync(`CONVERSE_ACCOUNT_ENCRYPTION_KEY_${account}`)
  if (existingKey) {
    return Buffer.from(existingKey, "base64")
  }
  logger.debug(`[Keychain] Creating account encryption key for account ${account}`)
  const newKey = Buffer.from(await getRandomBytesAsync(64))
  await setSecureItemAsync(`CONVERSE_ACCOUNT_ENCRYPTION_KEY_${account}`, newKey.toString("base64"))
  return newKey
}
