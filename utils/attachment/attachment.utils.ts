...
export function getMessageAttachmentLocalPath(messageId: string, filename: string) {
  const messageFolder = getMessagesAttachmentsLocalFolderPath(messageId)
...
export async function saveAttachmentLocally(attachment: LocalAttachment) {
  ...
  const attachmentLocalPath = getMessageAttachmentLocalPath(messageId, filename)
